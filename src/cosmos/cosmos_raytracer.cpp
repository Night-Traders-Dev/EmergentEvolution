...
struct SphereGPU {
    ...
    glm::vec4 color_emit;   // rgb = color (0-1), a = emissive
};
...
static glm::vec3 body_color_vec3(const CelestialBody& b) {
    ...
    case CTYPE_BLACK_HOLE: return {0.078f, 0.0f, 0.157f};
...
void CosmosRaytracer::update_and_draw(VulkanContext& vk, VkCommandBuffer cmd,
    ...
    for (int i = 0; i < n; i++) {
        ...
        float emissive = 0.0f;
        if (b.type == CTYPE_STAR)       emissive = 1.5f;
        if (b.type == CTYPE_BLACK_HOLE) emissive = -1.0f;  // negative = black hole flag
